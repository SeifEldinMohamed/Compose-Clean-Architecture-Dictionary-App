    ...
    val flow = if (shouldFetch(data)) {
        ...
        if (hasInternetConnection()) {
            try {
                ...
                query().map { Resource.Success(it) } // to convert Flow<List<TrendingRepositoriesEntity>> to Flow<NetworkResult<List<TrendingRepositoriesEntity>>>
            ...
            Log.d("dic", "No Internet Connection")
            query().map { Resource.Error("No Internet Connection", it) }
        }
    }
